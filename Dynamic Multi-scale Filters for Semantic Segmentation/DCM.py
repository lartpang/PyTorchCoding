    ...
    def __init__(self, in_C, out_C):
        ...
        for k in self.ks:
            self.ger_kernel_branches.append(
                nn.Sequential(nn.AdaptiveAvgPool2d(k), nn.Conv2d(in_C, self.mid_C, kernel_size=1))
            )

        self.trans_branches = nn.ModuleList()
        self.fuse_inside_branches = nn.ModuleList()
        for i in range(len(self.ks)):
            self.trans_branches.append(nn.Conv2d(in_C, self.mid_C, kernel_size=1))
            self.fuse_inside_branches.append(nn.Conv2d(self.mid_C, self.mid_C, 1))
